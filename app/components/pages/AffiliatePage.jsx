...
export function AffiliatePage() {
  ...
  const referralPath = `/r/${affiliateCode}`;
  ...
  return (
    <div style={{ paddingTop: 40 }}>
      ...
      {!loggedIn && <GuestNotice page="Affiliate" onSignIn={openLoginModal} />}

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 24, opacity: loggedIn ? 1 : 0.4, pointerEvents: loggedIn ? "auto" : "none" }}>
        <div className="card" style={{ padding: 28 }}>
          <div style={{ fontFamily: "'Fredoka', sans-serif", fontSize: 13, fontWeight: 800, color: "var(--text3)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 18 }}>Your Referral Link</div>
          ...
          <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 8, marginBottom: 16 }}>
            ...
            {affiliateEditing ? (
              <>
                <input value={affiliateEditVal} onChange={e => setAffiliateEditVal(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, ""))} className="inp" style={{ width: "280px", minWidth: 0, padding: "12px 16px" }} maxLength="20" placeholder="New code" />
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", background: "var(--surface2)", border: "1px solid rgba(245,166,35,0.25)", borderRadius: 10 }}>
            ...
            <button
              ...
              disabled={claimLoading || affiliateStats.earnings < 1}
            >
        ...
        <div className="card" style={{ padding: 24, display: "flex", flexDirection: "column" }}>
          <div style={{ fontFamily: "'Fredoka', sans-serif", fontSize: 16, fontWeight: 800, color: "var(--text)", marginBottom: 16 }}>Affiliated Users</div>
          {affiliateStats.users.length === 0 ? (
